...
class EMHelper {
    ...
    /*  public static function megaOgogo($model, $attribute, $htmlOptions = array(), $fieldType = 'textField') {
      ...
      $output .= "<div class='tab-pane active' id='" . $sid . $lang . "'>";
      if ($fieldType == 'ckeditor') {
      $output .= Yii::app()->controller->widget('application.extensions.eckeditor.ECKEditor', array('model' => $model, 'attribute' => $attribute));
      } else {
      $output .= CHtml::$fieldType($model, $attribute, $htmlOptions);
      }
      $output .= "</div>";
      } else {
    ...
    public static function megaOgogo($model, $attribute, $htmlOptions = array(), $fieldType = 'textField') {
        ...
        if ((count($languages) > 1) AND in_array($attribute, $model->translated_attributes)) {
            ...
            echo "<ul class='nav nav-tabs easy-multilanguage-tabs' style='margin-bottom:0px; border-bottom:none;'>";
            foreach ($languages as $lang => $langName) {
                $isActive = ($model->default_language == $lang) ? 'active' : '';

                echo "<li class='{$isActive}'><a href='#" . $sid . $lang . "' data-toggle='tab'>{$langName}</a></li>";
            }
            echo "</ul>";
            ...
            foreach ($languages as $lang => $langName) {
                if ($lang == $model->default_language) {
                    echo "<div class='tab-pane active' id='" . $sid . $lang . "'>";
                    if ($fieldType == 'ckeditor') {
                        Yii::app()->controller->widget('application.extensions.eckeditor.ECKEditor', array('model' => $model, 'attribute' => $attribute));
                    } else {
                        echo CHtml::$fieldType($model, $attribute, $htmlOptions);
                    }
                    echo "</div>";
                } else {
                    echo "<div class='tab-pane' id='" . $sid . $lang . "'>";
                    if ($fieldType == 'ckeditor')
                        Yii::app()->controller->widget('application.extensions.eckeditor.ECKEditor', array('model' => $model, 'attribute' => $attribute . '_' . $lang));
                    else
                        echo CHtml::$fieldType($model, $attribute . '_' . $lang, $htmlOptions);
                    echo "</div>";
                }
            }
            echo "</div>";
    ...
    public static function catchLanguage() {
        ...
        if ($_POST['setting_language'] && $_POST['setting_language'] != '') {
            $lang = $_POST['setting_language'];
            $MultilangReturnUrl = self::createMultilanguageReturnUrl($lang);
            var_dump($MultilangReturnUrl);
            Yii::app()->controller->redirect($MultilangReturnUrl);
        }
    ...
    public static function createMultilanguageReturnUrl($lang) {
      ...
      //echo "sss";
        return Yii::app()->controller->createUrl(Yii::app()->controller->action->id, $arr);
    }
